            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
                {menuItems.map((item) => (
                    ...
                    >
                        <div 
                            key={item.id} 
                            className='bg-white rounded-2xl p-4 sm:p-6 shadow-md font-barlow cursor-pointer transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 hover:scale-[1.03]'
                        >
                            <img 
                                src={item.img} 
                                alt={item.name}
                                className='w-full h-40 md:h-48 object-contain mb-6 transition-transform duration-300 group-hover:scale-105' 
                            />
